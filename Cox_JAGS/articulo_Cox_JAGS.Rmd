...
feat_DB <- read.csv(file="../Cox_JAGS/larynx_hot_enc.csv")
...
\begin{itemize}
\item El riesgo base tiene 
\\
\item $h(t_i|\theta) \propto  \displaystyle (\sum^{K}_{m=1} \lambda_m I_{(a_{m-1},a_m ]}(t_i)) \ e^{\pmb{\beta}^T\pmb{x}}$
\\
\item $L(\theta|\pmb{x}) = \displaystyle \prod_{m}  h(t_i|\theta)^{\delta_m} \ S(t_i |\theta)$
\end{itemize}

De la primera relación se sigue que $\Lambda(t_i|\theta) = \displaystyle e^{\pmb{\beta}^T\pmb{x}} \int^{t_i}_{0}\sum^{K}_{m=1} \lambda_m I_{(a_{m-1},a_m ]}(t_i) \ dt$. Por otro lado, consideremos la regla de asoción 

Sea $\left \{    1,\ldots K   \right \}\ni m_i  = \# \text{ de intervalo en el cae } t_i$. Claramente la expresión para el riego base de puede simplificar en: 
