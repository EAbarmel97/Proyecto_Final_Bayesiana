...
for (i in 1:length(carga_viral$patient)) {
  reg = carga_viral$patient[i]
  datos_1 = carga_viral %>% filter(patient == reg)
...
carga_viral$rna_v <- factor(ifelse(carga_viral$rna_v<=50, "CV indetectable", 
                                  ifelse(carga_viral$rna_v<500, "CV baja", ifelse(carga_viral<40000, "CV media", "CV alta"))))
...
for (i in 1:length(tratamiento_art$patient)) { #hice cambio 
  reg = tratamiento_art$patient[i]
...
for (i in 1:length(tratamiento_art$patient)){ #hice cambios
  reg = tratamiento_art$patient[i]
  datos_1 = tratamiento_art %>%  filter(tratamiento_art$patient==reg)
  datos_1$eliminar=NA
