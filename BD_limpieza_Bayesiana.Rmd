...
art_ed             <-- fecha de termino del tratamiento  - si no hay fecha de termino el paciente continuo con dicho tratamiento -
...
# Con un for vamos a revisar cada entrada de la variable "rna_d" para obtener la fecha minima que sera la cual nos dejara al primer conteo de CV. Tardara un poco en ejecutarse, paciencia.

for (i in 1:length(carga_viral$patient)) {
  reg = patients[i]
  datos_1 = subset(carga_viral, carga_viral$patient==reg)
  datos_1$eliminar=NA
...
# Esta comparacion es aun mas tardada, mucha mas paciencia
...
for (i in 1:length(tratamiento_art$patient)) {
  reg = patients[i]
  datos_1 = subset(tratamiento_art, tratamiento_art$patient==reg)
  datos_1$eliminar=NA
...
Finalmente con un unico registro por cada paciente dentro de cada tabla procedemos a unirlas para generar nuestra "BD" con la que haremos el modelo
...
sum(is.na(b1$cd4_d))
sum(is.na(b1$cd4_v))
# se generaron algunos na's al unir tabla lo que nos indica que tenemos pacientes con faltantes en los registros de CV, por esta razon no podemos usarlos pues son valores que no entran dentro de lo que necesitamos para el estudio y los omitimos por falta de informacion
b1 <- na.omit(b1)
