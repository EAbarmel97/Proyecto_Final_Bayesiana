...
data.jags <- list(n = nrow(larynx), m=length(a)-1, delta=larynx$delta, time=larynx$time, X=X[,-1], 
...
param.jags <-c("beta", "lambda")   
```


```{r}
#compilación del modelo 
modelo_compilado <- jags.model(data = data.jags,file = "modelo_juguete.txt", inits = init.jags, n.chains = 3)
```
